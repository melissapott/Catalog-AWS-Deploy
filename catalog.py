...
from database_setup import Base, Person, Category, Item
import random
import string
...
UPLOAD_FOLDER = '/var/www/Catalog/images'
...
def itemsByCategory(category_id):
    ...
    return render_template('category.html', category=category, items=items,
                           user_status=userStatus(category.person_id))
...
def editCategory(category_id):
    ...
    if 'user_id' not in login_session:
        ...
        return redirect(url_for('showLogin'))
    if login_session['user_id'] != category.person_id:
...
def deleteCategory(category_id):
    ...
    if ('user_id' not in login_session or
            category.person_id != login_session['user_id']):
        flash('You must be logged in as author of this category to edit it.')
        return redirect(url_for('itemsByCategory', category_id=category_id))
...
def itemDetail(item_id):
    ...
    return render_template('itemdetail.html', item=item, category=category,
                           user_status=userStatus(item.person_id))
...
def editItem(item_id):
    ...
    if 'user_id' not in login_session:
        ...
        return redirect(url_for('showLogin'))
    if login_session['user_id'] != item.person_id:
